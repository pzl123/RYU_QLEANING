...
class ecmp(app_manager.RyuApp):
    ...
    _CONTEXTS = {
        "topoinfo": topoinfo.topoinfo
    }

    def __init__(self, *args, **kwargs):
        super(ecmp, self).__init__(*args, **kwargs)
        self.arp_handler = kwargs["topoinfo"]
        self.datapaths = {}

    @set_ev_cls(ofp_event.EventOFPSwitchFeatures, CONFIG_DISPATCHER)
    def switch_features_handler(self, ev):
        #self.logger.info('--------触发Table-Miss中----------')
    ...
    def get_sw(self, dpid, in_port, src, dst):
        ...
        src_location = self.arp_handler.get_host_location(src)#arp_handler 使用topoinfo内部的函数
        if in_port in self.arp_handler.access_ports[dpid]:
            if (dpid,  in_port) == src_location:
                src_sw = src_location[0]#src_location[0]是sw
            else:
                return None
